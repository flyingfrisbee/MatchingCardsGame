...
import com.giovann.mymemorysecond.utils.*
...
class CreateActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode != PICK_PHOTO_CODE || resultCode != Activity.RESULT_OK || data == null) {
            Log.w(TAG, "Didn't get data back from the launched activity, user likely canceled flow")
            return
        }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == READ_EXTERNAL_PHOTOS_CODE) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(this, "In order to create a custom game,\nyou need to provide access to your photos", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        for ((index, photoUri) in imageUris.withIndex()) {
            ...
            photoReference.putBytes(imageByteArray)
                .continueWithTask { photoUploadTask ->
                    ...
                    pbUploading.progress = uploadedImageUrls.size * 100 / imageUris.size
    ...
    private fun getImageByteArray(photoUri: Uri): ByteArray {
        ...
        val scaledBitmap = BitmapScaler.scaleToFitWidth(originalBitmap, 250)
        Log.i(TAG, "Scaled width ${scaledBitmap.width} and height ${scaledBitmap.height}")
        val byteOutputStream = ByteArrayOutputStream()
        scaledBitmap.compress(Bitmap.CompressFormat.JPEG, 80, byteOutputStream)
